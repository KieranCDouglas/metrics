...
constrained <-lm(chow_clean, )
)

constrained_model <- lm(data = constr_chow, ln_rent~ln_mem+ln_mult+ln_access)
...
for (yr in years) {
  subset_year <- constr_chow |> filter(year == yr)
  model_year <- lm(ln_rent ~ ln_mem + ln_mult + ln_access, data = subset_year)
  rss_sep <- rss_sep + sum(resid(model_year)^2)
  n_sep <- n_sep + nrow(subset_year)
}

# extractingt the important stats
rss_pooled <- sum(resid(constrained_model)^2)
n_pooled <- nrow(constr_chow)
k <- length(coef(constrained_model))
...
F_stat <- ((rss_pooled - rss_sep) / df1) / (rss_sep / df2)
